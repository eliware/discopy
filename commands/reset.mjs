import plan from '../src/reset/plan.mjs';
...
export default async function ({ log, msg }, interaction){
  ...
  if(!isAdmin){ return interaction.reply({ content: 'You must have Administrator permission to run /reset.', flags: 1<<6 }); }
  ...
  // Step 5: Finalize
  try{ await interaction.followUp({ content: `reset ${dryRun? 'dry-run completed' : 'completed'}. Results: ${JSON.stringify(results)}. Guild defaults: ${JSON.stringify(guildDefaultsResult)}. Defaults created: ${JSON.stringify(created)}`, ephemeral: false }); }catch(e){}
