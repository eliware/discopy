...
export default async function createDefaults(guild, maps, dryRun, log){
  ...
  try{
    ...
    const textCat = await guild.channels.create('Text Channels', { type: 4, reason: 'nuke: create defaults' });
    created.categories.push(textCat.id);
    const voiceCat = await guild.channels.create('Voice Channels', { type: 4, reason: 'nuke: create defaults' });
    created.categories.push(voiceCat.id);
    // text channel general
    const general = await guild.channels.create('general', { type: 0, parent: textCat.id, reason: 'nuke: create defaults' });
    created.channels.push(general.id);
    // voice channel
    const gvc = await guild.channels.create('General VC', { type: 2, parent: voiceCat.id, reason: 'nuke: create defaults' });
